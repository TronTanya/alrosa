/**
 * Семантический подбор через Yandex Foundation Models (Алиса / YandexGPT).
 *
 * Локально: YANDEX_CLOUD_API_KEY в .env — Vite проксирует /yandex-llm-api без утечки ключа в бандл.
 * Иначе: VITE_YANDEX_CLOUD_API_KEY (ключ попадёт в клиент — только для прототипов).
 */
...
const SYSTEM_PROMPT = `Ты помощник корпоративного портала обучения «Алроса ИТ». По запросу пользователя и каталогу сущностей выбери наиболее релевантные пункты. Если в каталоге есть материалы про вузы, колледжи, ДПО или платформы — учитывай их наравне с прочими.
