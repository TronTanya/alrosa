...
import { Search, Filter, Download, RefreshCw, ChevronDown, ArrowUpDown, ChevronUp } from "lucide-react";
...
const logs: LogEntry[] = [
  ...
  { id: 3,  time: "10:34:21", user: "Анна Смирнова",      role: "L&D",     action: "Экспорт отчёта HR-аналитики — 312 сотрудников",          module: "LMS",          status: "success", ip: "10.0.0.45"    },
  ...
  { id: 9,  time: "10:11:28", user: "GigaChat API",       role: "Bot",     action: "Обновление векторной базы знаний — +148 документов",      module: "AI Engine",    status: "success", ip: "internal"     },
    ...
    <div className="glass-card" style={{ padding: "24px" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "18px", gap: "12px", flexWrap: "wrap" }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
            ...
            <h3 style={{ fontSize: "15px", fontWeight: "700", color: "#000000", margin: 0 }}>Последние действия в системе</h3>
            {errorCount > 0 && (
              <div style={{ padding: "3px 9px", borderRadius: "20px", background: "rgba(255,71,87,.15)", border: "1px solid rgba(255,71,87,.3)", fontSize: "10px", fontWeight: "700", color: "#FF4757" }}>
                {errorCount} ошибок
              </div>
            )}
            {warningCount > 0 && (
              <div style={{ padding: "3px 9px", borderRadius: "20px", background: "rgba(255,181,71,.13)", border: "1px solid rgba(255,181,71,.28)", fontSize: "10px", fontWeight: "700", color: "#FFB547" }}>
                {warningCount} предупрежд.
              </div>
            )}
          </div>
          <p style={{ fontSize: "12px", color: "rgba(0,0,0,0.55)", margin: 0 }}>Аудит · Мониторинг в реальном времени · Сегодня 30.03.2026</p>
        </div>
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", alignItems: "center" }}>
          ...
          <button type="button" style={{ display: "flex", alignItems: "center", gap: "5px", padding: "7px 12px", borderRadius: "9px", background: "rgba(227,0,11,0.08)", border: "1px solid rgba(227,0,11,0.25)", color: "#e3000b", fontSize: "11.5px", cursor: "pointer", fontFamily: "var(--font-sans)", fontWeight: "600" }}>
      ...
      <div style={{ display: "flex", gap: "4px", marginBottom: "16px", overflowX: "auto" }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: "5px", padding: "5px 12px", borderRadius: "20px", background: "rgba(0,196,160,.08)", border: "1px solid rgba(0,196,160,.18)" }}>
          ...
          <span style={{ fontSize: "11px", color: "#00C4A0", fontWeight: "600" }}>Live</span>
        </div>
      </div>
            ...
            <tr>
              {[
                ...
                <th key={i} onClick={key ? () => toggleSort(key) : undefined}
                  style={{ textAlign: "left", padding: "0 12px 10px", fontSize: "10px", fontWeight: "600", color: "rgba(0,0,0,0.45)", letterSpacing: ".7px", textTransform: "uppercase", cursor: key ? "pointer" : "default", width: w, whiteSpace: "nowrap" }}>
                ...
                <tr key={log.id} onMouseEnter={() => setHovRow(log.id)} onMouseLeave={() => setHovRow(null)}>
                  ...
                  <td style={{ padding: "10px 12px", background: isH ? "rgba(129,208,245,0.08)" : isErr ? "rgba(227,0,11,0.06)" : "transparent", borderTop: "1px solid rgba(0,0,0,0.06)", borderRadius: "12px 0 0 12px", transition: "background .15s" }}>
                    <div style={{ display: "flex", flexDirection: "column", gap: "1px" }}>
                      <span style={{ fontSize: "12px", fontWeight: "700", color: isErr ? "#e3000b" : "#000000", fontVariantNumeric: "tabular-nums", fontFamily: "monospace" }}>{log.time}</span>
                  ...
                  <td style={{ padding: "10px 12px", background: isH ? "rgba(129,208,245,0.08)" : isErr ? "rgba(227,0,11,0.06)" : "transparent", borderTop: "1px solid rgba(0,0,0,0.06)", transition: "background .15s" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "7px" }}>
                      <div style={{ width: "28px", height: "28px", borderRadius: "50%", background: log.role === "Bot" ? "linear-gradient(135deg,#e3000b,#81d0f5)" : roleGradients[idx % roleGradients.length], display: "flex", alignItems: "center", justifyContent: "center", fontSize: "9px", fontWeight: "800", color: "#000000", flexShrink: 0 }}>
                        {log.role === "Bot" ? "🤖" : log.user.split(" ").map(w => w[0]).slice(0, 2).join("")}
                      </div>
                      <div>
                        <div style={{ fontSize: "12px", fontWeight: "600", color: isH ? "#000000" : "rgba(0,0,0,0.85)", whiteSpace: "nowrap" }}>{log.user.length > 17 ? log.user.split(" ")[0] + " " + log.user.split(" ")[1]?.[0] + "." : log.user}</div>
                  ...
                  <td style={{ padding: "10px 12px", background: isH ? "rgba(129,208,245,0.08)" : isErr ? "rgba(227,0,11,0.06)" : "transparent", borderTop: "1px solid rgba(0,0,0,0.06)", transition: "background .15s" }}>
                    <div style={{ display: "inline-flex", alignItems: "center", gap: "5px", padding: "4px 10px", borderRadius: "20px", background: `${mc}18`, border: `1px solid ${mc}30` }}>
                      ...
                      <span style={{ fontSize: "10.5px", fontWeight: "600", color: mc, whiteSpace: "nowrap" }}>{log.module}</span>
                    </div>
                  </td>
                  {/* IP */}
                  <td style={{ padding: "10px 12px", background: isH ? "rgba(129,208,245,0.08)" : isErr ? "rgba(227,0,11,0.06)" : "transparent", borderTop: "1px solid rgba(0,0,0,0.06)", transition: "background .15s" }}>
